...
Phase 1–3, **Phase 4.1–4.12** và **Phase 5.1–5.2** đã hoàn tất Windows-first. **Phase 5.3–5.6**, **Phase 6.1** và **Phase 6.2** đã triển khai: Home có Start/Retry theo native state, **Mở bán hàng** dùng route CoffeePOS thật trên dynamic runtime origin, store đã cài auto-start runtime, Close/Alt+F4 có bounded shutdown, Chẩn đoán kiểm health riêng theo component, và runtime POS Windows đã chuyển sang Caddy + PHP FastCGI workers + OPcache với lifecycle/health blocking work ngoài UI thread. Focused/lightweight automated checks đã pass; native/manual acceptance của các milestone này để người dùng smoke-test trước khi đánh dấu hoàn thành theo Definition of Done.

...
Phase 1–3, **Phase 4.1–4.12** và **Phase 5.1–5.2** đã hoàn tất Windows-first. **Phase 5.3–5.6** và **Phase 6.1–6.3** đã triển khai: Home có Start/Retry theo native state, **Mở bán hàng** dùng route CoffeePOS thật trên dynamic runtime origin, store đã cài auto-start runtime, Minimize ẩn shell xuống tray còn Close/Alt+F4 và tray **Thoát hoàn toàn** dùng bounded-shutdown runtime, Chẩn đoán kiểm health riêng theo component, runtime POS Windows dùng Caddy + PHP FastCGI workers + OPcache, và **Hệ thống → Sửa chữa** có native repair plan/apply, atomic managed repair, crash recovery và credential safety gates. Focused/lightweight automated checks đã pass; native/manual acceptance của các milestone này để người dùng smoke-test trước khi đánh dấu hoàn thành theo Definition of Done.

# Provisioning — Phase 3, Phase 4.x và onboarding Phase 5.2
...
## Phase 5.2 onboarding profile và administrator credential

Fresh store phải commit setup profile trước side effect provisioning đầu tiên. `config/app.json` giữ schema `1` và có optional `setup_admin_username` / `setup_admin_email`; file cũ không có hai field này vẫn hợp lệ. `store_name`, username và email là metadata setup/recovery. Password không được ghi vào JSON: `save_setup_profile` lưu password do người dùng đặt vào `config/wordpress-admin.secret` bằng Windows DPAPI, rồi `provision_wordpress` chỉ chạy khi fresh store đã có cả profile và protected password.

Fresh administrator contract: username 3–60 ASCII chữ/số/`.`/`_`/`-`; email tối đa 100 ký tự với local-part giới hạn ở chữ/số/`.`/`+`/`_`/`-` để `sanitize_email` không đổi giá trị; password 12–128 ký tự và không có control character. Fresh store name dùng canonical subset mà CoffeePOS `sanitize_text_field` giữ nguyên: sau trim dài 1–80 ký tự, không control character, HTML bracket, repeated ASCII space hoặc `%XX` byte escape. WordPress có thể HTML-escape `blogname`, nên bootstrap so với canonical value từ `sanitize_option('blogname', ...)`; CoffeePOS Settings/health vẫn trả tên nghiệp vụ nguyên bản. Bootstrap truyền username/email/password qua child environment của bounded pinned-PHP process, không qua argv/log. Exact initial user/email/administrator role/`wp_check_password()`/blogname phải được verify cho tới khi journal commit `wordpress_installed`, kể cả retry sau khi WordPress tables đã tồn tại.

Profile chỉ editable khi native inspection còn `not_installed`. Khi thay password trước provisioning, native atomically stage password mới vào `config/wordpress-admin.pending.secret`, commit app config, rồi promote staged DPAPI secret sang `wordpress-admin.secret`. Pending marker không bị xóa trên metadata-save failure; nó chỉ biến mất sau active-secret promotion + read-back verification thành công. Vì vậy mọi crash/failure giữa metadata và secret đều giữ `password_configured=false` và `provision_wordpress` bị chặn cho tới khi người dùng save password lại. Một khi journal/site/database cho thấy provisioning đã bắt đầu, Desktop không được thay username/password ngầm. `ProvisioningJournal.admin_username` phải khớp profile khi retry; mismatch bị từ chối. Store Phase 4.x cũ không có onboarding metadata dùng fallback `coffeepos_admin` / `admin@coffeepos.local` và protected generated secret hiện có, nên retry/completed-store startup không reset credential.

Tên cửa hàng fresh được seed vào hai nguồn runtime thật: WordPress `blogname` trong `wp_install()` và CoffeePOS `Settings::OPTION_STORE_NAME` trong activation baseline. Desktop đặt current user là initial administrator rồi gọi plugin-owned `Settings::update()` và verify `Settings::getStoreName()`. Seed chỉ chạy khi activation baseline cần apply; rerun sau baseline không overwrite setting plugin đã tồn tại. Machine-health `store.name` tiếp tục đọc từ CoffeePOS Settings, không từ Desktop app config.

`get_setup_info` chỉ trả non-secret setup metadata, `password_configured` và `editable`. `copy_admin_password` là IPC hẹp cho main shell: native load protected initial secret rồi ghi thẳng vào Windows clipboard; plaintext không được serialize trở lại WebView. Đây là **initial administrator password** snapshot; nếu password được đổi sau này trong WordPress, Desktop không giả định snapshot đó vẫn là current password.
